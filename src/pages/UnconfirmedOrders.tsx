...
import { getUnconfirmedOrders } from "../api/services";
import { calculateOrderTotalPrice, formatDate } from "../lib/helpers";

function UnconfirmedOrders() {
  const [unconfirmedOrdersData, setUnconfirmedOrdersData] = useState<{
    data: IOrder[];
    count: number;
  }>({ data: [], count: 0 });
  ...
  const perPage = 5;
  const pageCount = Math.ceil(unconfirmedOrdersData.count / perPage);
    ...
    function () {
      async function getOrders(startIndex: number, endIndex: number) {
        ...
        try {
          ...
          const { data, count } = await getUnconfirmedOrders(
            startIndex,
            endIndex
          );
          setUnconfirmedOrdersData({ data: data as IOrder[], count });
        } catch (error) {
          console.error("Error fetching data:", error);
          setUnconfirmedOrdersData({ data: [], count: 0 });
        } finally {
          setIsLoading(false);
        }
      }
  ...
  return (
    <div className="bg-slate-900 w-full h-screen text-white overflow-y-hidden">
      <div className="w-[80%] h-screen m-auto overflow-y-scroll scrollbar-hide">
        <p className="text-center font-semibold text-4xl p-1 bg-gradient-to-r from-orange-500 to-orange-500 bg-clip-text text-transparent">
          Unconfirmed orders
        </p>
        {!isLoading ? (
          unconfirmedOrdersData?.data.length > 0 ? (
            <>
              <ul className="text-black font-semibold">
                <li className="flex flex-row items-center justify-between bg-gradient-to-r from-orange-400 to-orange-500 mb-4 mt-4 rounded-lg p-1">
              ...
              <ul>
                {unconfirmedOrdersData?.data.map((order: IOrder) => (
                  <Link to={`/order/${order.id}`} key={order.id}>
                    <li className="shadow-2xl hover:scale-95 cursor-pointer duration-150 flex flex-row items-center justify-between bg-gradient-to-r from-orange-400 to-orange-500 mb-4 rounded-lg h-[130px]">
              ...
              <div className="flex justify-between mt-3">
                ...
                <button
                  className="bg-gray-500 rounded-lg px-4 py-2 font-semibold"
                  disabled={unconfirmedOrdersData?.data.length <= perPage}
                  onClick={() => setPage(page + 1)}
                >
                  Next
                </button>
